...
def walk_the_walk(filename, steps, source):
    def find_neighbors(space):
        ...
        for delta in MOVEMENTS:
            ...
            array[int(new_space.real) % height][int(new_space.imag) % width] == ".":
    ...
    queue = queue.union({source + 1, source - 1, source + 1j, source - 1j})
    new_queue = set()
    visited = queue.copy()
...
print(walk_the_walk("day_21.txt", 20))
