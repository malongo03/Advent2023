"""
Module to solve the first star of Day 1 of the Advent of Code 2023, which is the
1st star overall.
"""
def unscamble_calibration(file):
    """
    Takes in an amended elf trebuchet calibration (see Day 1 of Advent of Code
    2023), unamends it by finding the proper calibration numbers, and sums these
    calibration numbers.

    Inputs:
        file(str): the name of the input file

    Returns [int]
    """
    # Process input file into form usable by algorithm
    f = open(file, encoding = "utf-8")
    text = f.read()
    text = text.split()

    answer = 0

    for line in text:
        first_digit = None
        last_digit = None
        for cha in line:
            if cha.isdigit():
                last_digit = int(cha)
                if first_digit is None:
                    first_digit = int(cha)
        assert first_digit is not None and last_digit is not None
        answer += first_digit * 10 + last_digit

    return answer

print(unscamble_calibration("day_1.txt"))
